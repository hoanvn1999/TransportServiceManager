...
std::vector<PossiblePlate> detectPlatesInScene(cv::Mat &imgOriginalScene) {
    ...
    std::cout << std::endl << vectorOfPossiblePlates.size() << " vung co the la bien so" << std::endl;       // 13 with MCLRNF1 image
    ...
    for (unsigned int i = 0; i < vectorOfPossiblePlates.size(); i++) {
        ...
        std::cout << "vung kha thi:  " << i << ", click va chon phim bat ki de tiep tuc . . ." << std::endl;

        cv::imshow("4b", vectorOfPossiblePlates[i].imgPlate);
        cv::waitKey(0);
    }
    std::cout << std::endl << "phat hien vung hoan chinh, click va chon phim bat ki de tiep tuc . . ." << std::endl << std::endl;
    cv::waitKey(0);
#endif	// SHOW_STEPS
...
std::vector<PossibleChar> findPossibleCharsInScene(cv::Mat &imgThresh) {
    ...
    std::cout << "Buoc 2 - intCountOfValidPossibleChars = " << intCountOfPossibleChars << std::endl;        // 131 with MCLRNF1 image
