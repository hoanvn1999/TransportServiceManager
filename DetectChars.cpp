...
bool loadKNNDataAndTrainKNN(void) {
    ...
    if (fsClassifications.isOpened() == false) {                                                        // if the file was not opened successfully
        std::cout << "Loi: Khong the mo truy cap toi bo nho tri tue nhan tao\n\n";        // show error message
    ...
    if (fsTrainingImages.isOpened() == false) {                                                 // if the file was not opened successfully
        std::cout << "Loi: Khong the mo thong tin anh dao tao\n\n";         // show error message
        return(false);                                                                          // and exit program
    }
...
#ifdef SHOW_STEPS
            std::cout << "Ki tu duoc tim thay " << intPlateCounter << " = (none), click vao anh va chon bat ki phim nao de tiep tuc . . ." << std::endl;
            intPlateCounter++;
            cv::destroyWindow("8");
            cv::destroyWindow("9");
            cv::destroyWindow("10");
...
#ifdef SHOW_STEPS
        std::cout << "Ki tu duoc tim thay " << intPlateCounter << " = " << possiblePlate.strChars << ", click vao anh va chon phim bat ki de tiep tuc . . ." << std::endl;
        intPlateCounter++;
        cv::waitKey(0);
#endif	// SHOW_STEPS
...
#ifdef SHOW_STEPS
    std::cout << std::endl << "hoan thanh nhan dien, click hoac chon bat ki phim nao de tiep tuc . . ." << std::endl;
    cv::waitKey(0);
#endif	// SHOW_STEPS
